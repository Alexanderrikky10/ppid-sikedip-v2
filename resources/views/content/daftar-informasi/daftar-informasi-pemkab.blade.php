...
@section('content')
    ...
    @php
        ...
        $deskripsi = 'Temukan seluruh daftar informasi publik yang dikelola oleh Pemerintah Kabupaten/Kota di Kalimantan Barat.';
        $badgeText = 'PPID Utama Kabupaten/Kota';
        $logoUrl = asset('images/office-building.png'); // Default Logo Lokal
        ...
        if ($isSpecificOpd) {
            ...
            $badgeText = 'PPID Pelaksana Kab/Kota';
    ...
    $percent = $total > 0 ? ($val / $total) * 100 : 0;
            ...
            {{-- 5. GRID PEMKAB LAINNYA (DI BAWAH) --}}
            @if(isset($PemkabUtama))
                <div class="border-t border-gray-200 pt-10 mt-10" x-data="{ searchPemkab: '', activeTab: 'all' }">
                    <h2 class="text-2xl font-bold text-gray-800 mb-6 text-center">Jelajahi Pemkab/Pemkot Lainnya</h2>

                    {{-- Search Bar untuk Grid Bawah --}}
                    <div class="max-w-md mx-auto mb-8 relative">
                        <i class="fa-solid fa-magnifying-glass absolute left-3 top-3 text-gray-400"></i>
                        <input type="text" x-model="searchPemkab" placeholder="Cari kabupaten/kota..."
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                        @foreach($PemkabUtama as $pemkab)
                                {{-- LOGIKA URL GAMBAR (MINIO / LOCAL) --}}
                                @php
                                    $pemkabLogoUrl = asset('images/office-building.png'); // Default Logo

                                    if (!empty($pemkab->images)) {
                                        if (filter_var($pemkab->images, FILTER_VALIDATE_URL)) {
                                            $pemkabLogoUrl = $pemkab->images;
                                        } else {
                                            try {
                                                $pemkabLogoUrl = Storage::disk('minio')->temporaryUrl(
                                                    $pemkab->images,
                                                    now()->addMinutes(60)
                                                );
                                            } catch (\Exception $e) {
                                                try {
                                                    $pemkabLogoUrl = Storage::url($pemkab->images);
                                                } catch (\Exception $ex) {
                                                    $pemkabLogoUrl = asset('images/office-building.png');
                                                }
                                            }
                                        }
                                    }
                            ...
                            <a href="{{ route('daftar-informasi-pemkab.list', ['slug' => $pemkab->slug]) }}"
                                    ...
                                    x-show="searchPemkab === '' || '{{ strtolower($pemkab->nama_perangkat_daerah) }}'.includes(searchPemkab.toLowerCase())">
                                    ...
                                    <div
                                        ...
                                        <img src="{{ $pemkabLogoUrl }}" alt="Logo {{ $pemkab->nama_perangkat_daerah }}"
